...
### 4.8 Economy and adaptation: making money mean something

A follow-on set of changes teaches the opponent *why* it is fighting: its budget
has a physical, defendable source, and the enemy's does too. Everything added is
RED's own economy or the generic per-category value of targets already shown to
RED — no BLUE economy figure is ever surfaced (`test_intel_leak.py` still
passes).

* **Economic intelligence in the own-forces brief (`intel.py` + `operations.py`).**
  The `[RESOURCES]` block now states that income is generated by the bases RED
  holds and their buildings (oil/derrick/factory/fuel/ammo/warehouse) and that a
  base or building lost is future budget lost. Each `[OWN BASES]` line carries
  that base's own `income=N/turn` (control point plus its income buildings,
  scaled by RED's income multiplier, mirroring `game.income.Income`), so the
  model can see which bases fund the force.
* **Enemy income-building targets tagged with their value (`operations.py`).**
  An identified enemy objective that is an income-generating building carries an
  `income~N/turn` tag taken from the `REWARDS` table by category (oil ~10,
  derrick ~8, factory ~2.5, fuel/ammo/warehouse ~2, farp ~1); categories not in
  `REWARDS` get no tag. A deep strike on a derrick or oil rig therefore reads as
  a high-value blow to the enemy budget rather than filler.
* **Economic doctrine in the prompts (`activeprompt.py`).** The command-intent
  briefing explains that money sustains the war: protect the bases/buildings that
  fund the force, striking the enemy's oil/derricks/factories/fuel/ammo cuts
  their future income and forces, and capturing territory gains the income
  buildings on it. The logistics briefing reinforces holding and advancing a
  front to gain that territory.
* **Front holding and force-composition doctrine (`activeprompt.py`).** The
  logistics briefing adds a `FORCE MIX` rule: to break through, mass heavy armour
  and anti-armour; to survive enemy air over the front, include AAA and
  SAM/SHORAD cover — matched to the situation and to buyable unit types.
* **Learning from ground losses (`debrief.py`).** The `next_turn:` guidance now
  reads air and ground losses separately: ground units lost to enemy aircraft ⇒
  add AAA/SAM/SHORAD cover and contest the air; ground units lost to enemy ground
...
| `test_debrief.py` | The after-action summary (§4.6, §4.7, §4.8): `classify_threat` mapping, cause attribution and the `UNKNOWN` remainder bucket, RED-perspective loss/kill counting, serialization round-trip and garbage tolerance, the `render_compact` length bound, the actionable `next_turn:` guidance keyed off dominant loss causes — including the ground-loss directives (ground units lost to enemy air ⇒ AAA/SAM/SHORAD, to ground fire ⇒ heavier armour), present when actionable and absent otherwise — and, same as `test_intel_leak.py`, that the rendered block leaks no BLUE information. |
...
| `test_operations_brief.py` | The richer ACTIVE brief. Same `REALISTIC` fairness boundary as `test_intel_leak.py` but for the extra ACTIVE fields, plus per-stage intel-leak sentinels — no BLUE budget, inventory, squadron or base detail in any stage prompt. Also (§4.8) the enemy income-building target tag: `TargetView.render` shows `income~N/turn` only for income-generating categories, and `_target_income` derives the value from the `REWARDS` table (derrick 8, oil 10, factory 2.5, ammo 2), returning `None` for non-income categories. |
